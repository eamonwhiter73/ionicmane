...
var CameraService = /** @class */ (function () {
        ...
        return this.camera.getPicture(options)
            ...
            if (newPath) {
                ...
                _this.file.readAsDataURL(filePath, fileName).then(function (data) {
                    ...
                    return new Promise(function (resolve, reject) {
                        storageRef = firebase.storage().ref('/profile/' + _this.username + '/' + image);
                        parseUpload = storageRef.putString(dataURL, 'data_url');
                        parseUpload.on('state_changed', function (_snapshot) {
                            // We could log the progress here IF necessary
                            console.log('snapshot progess ' + _snapshot);
    ...
    CameraService = __decorate([
        Injectable(),
        __metadata("design:paramtypes", [Storage, Transfer, Http, Platform, Camera, Crop, File, AngularFireDatabaseModule])
    ], CameraService);
